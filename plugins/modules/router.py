...
def _get_subnet_ids_from_ports(ports):
...
def _needs_update(cloud, module, router, net,
                  missing_port_ids,
    ...
    if router_ifs_cfg['external_fixed_ips']:
        ...
        if len(router['external_gateway_info']['external_fixed_ips']) > 1:
...
def _build_kwargs(cloud, module, router, net):
    kwargs = {
    ...
    if module.params['external_fixed_ips']:
        ...
        # no external fixed ips requested
        if router and len(router['external_gateway_info']['external_fixed_ips']) > 1:
            # but router has several external fixed ips
...
def _build_router_interface_config(module, cloud, filters=None):
    external_fixed_ips = []
    internal_subnets = []
    internal_ports = []
...
def main():
    ...
    try:
        ...
        router_ifs_cfg = _build_router_interface_config(module, cloud, filters)
        requested_subnet_ids = [subnet.id for subnet in router_ifs_cfg['internal_subnets']] + \
            _get_subnet_ids_from_ports(router_ifs_cfg['internal_ports'])
        requested_port_ids = [i['id'] for i in router_ifs_cfg['internal_ports']]

        if router:
            router_ifs_internal = cloud.list_router_interfaces(router, 'internal')
            existing_subnet_ids = _get_subnet_ids_from_ports(router_ifs_internal)
        ...
        if module.check_mode:
            ...
            if state == 'absent' and router:
                ...
                changed = _needs_update(cloud, module, router, net,
                                        missing_port_ids,
                                        requested_subnet_ids,
                                        existing_subnet_ids,
                                        router_ifs_cfg,
        ...
        if state == 'present':
            ...
            if not router:
                ...
                for port in router_ifs_cfg['internal_ports']:
                    cloud.add_router_interface(router, port_id=port.id)
                ...
                for missing_internal_port in router_ifs_cfg['internal_ports_missing']:
                    p = cloud.create_port(**missing_internal_port)
                    if p:
                        cloud.add_router_interface(router, port_id=p.id)
                ...
                if _needs_update(cloud, module, router, net,
                    ...
                    for port_id in missing_port_ids:
                        cloud.add_router_interface(router, port_id=port_id)
                        changed = True
            ...
            if not router:
                ...
                module.exit_json(changed=True, router=router)
